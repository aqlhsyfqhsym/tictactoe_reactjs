...
const TicTacToe = () => {
  ...
  //whenever user click on box, toggle will executed
  const toggle = (e, num) => {
    if (lock) {
      return 0;
    }
  ...
  const won = (winner) => {
    setLock(true);
    if (winner === "x") {
      titleRef.current.innerHTML = `Winner is &nbsp; <img src='${cross}' alt="cross" />`;
    } else {
      titleRef.current.innerHTML = `Winner is &nbsp; <img src='${circle}' alt="circle" />`;
    }
  };
